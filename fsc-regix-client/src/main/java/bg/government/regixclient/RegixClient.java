...
public class RegixClient {
     ...
     */
    static RegixClient createTestClient(InputStream keyStore, char[] keyStorePassword) {
        return createClient(keyStore, keyStorePassword, "https://regix-service-test.egov.bg/regix/RegiXEntryPoint.svc");
    }

    public static RegixClient createClient(InputStream keyStore, char[] keyStorePassword, String endpointAddress) {
        IRegiXEntryPoint entryPoint = new RegiXEntryPoint().getWSHttpBindingIRegiXEntryPoint(new AddressingFeature());
        WSBindingProvider bp = (WSBindingProvider) entryPoint;

        bp.getRequestContext()
                .put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, endpointAddress);
        RegixClient client = new RegixClient(entryPoint);
    ...
    protected RegixClient(IRegiXEntryPoint regix) {
        this.regix = regix;
    }
    
    protected void initTLS(InputStream keyStore, char[] keyStorePassword) {
